using DailyPXNotificationPENCOBatch.BLL;
using SIUtil;
...
namespace DailyPXNotificationPENCOBatch
{
    public class DailyPXNotificationPENCO
    {
        public void Run(string a_sScheduleName)
        {
            try
            {
                ...
                if (!ScheduleUtils.HasJobRun(a_sScheduleName))
                {
                    ...
                    var taskStatus = processor.ProcessPXNotificationPENCO();
