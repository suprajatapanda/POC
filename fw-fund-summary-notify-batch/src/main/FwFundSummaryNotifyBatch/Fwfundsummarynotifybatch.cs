using FWFundSummaryNotifyBatch.BLL;
using SIUtil;
...
namespace FWFundSummaryNotifyBatch
{
    public class Fwfundsummarynotifybatch
    {
        public void Run(string a_sScheduleName)
        {
            ...
            try
            {
                ...
                if (!ScheduleUtils.HasJobRun(a_sScheduleName))
                {
                    ...
                    var processor = new FWBend(fWBend);
                    taskStatus = processor.ProcessSendSummary();
            ...
            catch (Exception ex)
            {
                ...
                if (taskStatus != null) Utils.ParseError(taskStatus.errors);
